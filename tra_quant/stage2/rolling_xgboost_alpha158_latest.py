...
CACHE_DIR = BASE_DIR / "xgb_cache"
HANDLER_CACHE_DIR = CACHE_DIR / "handlers"
DETERMINISTIC_SEED = 42
LAUNCH_STAGGER_SECONDS = 0.1
...
def _recommended_nthread(gpu_slots) -> int:
    cpu_count = os.cpu_count() or 1
    slot_count = max(1, len(_normalize_gpu_slots(gpu_slots)) or 1)
    # Keep a small CPU reserve for data loading and system tasks while scaling per-GPU worker threads.
    return max(4, min(16, (max(1, cpu_count - 2) // slot_count)))
...
def _runtime_model_kwargs_override(model_kwargs_override: dict | None, gpu_slots) -> dict:
    runtime_override = copy.deepcopy(model_kwargs_override) if model_kwargs_override else {}
    runtime_override.setdefault("nthread", _recommended_nthread(gpu_slots))
...
def _train_tasks(task_list: list[dict], experiment_name: str, provider_uri: str, gpu_slots):
    ...
    with tempfile.TemporaryDirectory(prefix="xgb_tasks_") as temp_dir:
        ...
        while pending or running:
            while pending and available_slots:
                ...
                worker_threads = str(_recommended_nthread([gpu]))
                ...
                env.setdefault("NUMEXPR_NUM_THREADS", worker_threads)
                cmd = [
                    sys.executable,
                    str(script_path),
                    "train_task",
...
def run(
    ...
    runtime_model_kwargs_override = _runtime_model_kwargs_override(model_kwargs_override, gpu_slots)
    ...
    base_task = build_base_task(
        ...
        benchmark=benchmark,
        instruments=instruments,
        model_kwargs_override=runtime_model_kwargs_override,
    ...
    rolling_models_exp = (
        f"rolling_models_xgb_latest_{window_key}_{model_key}_{handler_class}_s{step}_k{topk}_d{n_drop}_a{account}"
        f"_r{str(risk_degree).replace('.', '')}{suffix}"
    )
    final_exp = (
        f"rolling_eval_xgb_latest_{window_key}_{model_key}_{handler_class}_s{step}_k{topk}_d{n_drop}_a{account}"
        f"_r{str(risk_degree).replace('.', '')}{suffix}"
    )
    ...
    print(f"runtime nthread: {runtime_model_kwargs_override.get('nthread')}")
    ...
    result_path = BASE_DIR / (
        f"rolling_result_xgb_{window_key}_{model_key}_{handler_class}_step{step}_k{topk}_d{n_drop}_a{account}"
        f"_r{str(risk_degree).replace('.', '')}{suffix}.txt"
    )
    cache_path = CACHE_DIR / (
        f"rolling_cache_xgb_{window_key}_{model_key}_{handler_class}_step{step}_k{topk}_d{n_drop}_a{account}"
...
def run_from_config(config_path: str):
    ...
    run(
        ...
        model_key=model.get("model_key", "xgb_base"),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run rolling Alpha158 + XGBoost experiments.")
    ...
    parser.add_argument("--model-key", dest="model_key", default="xgb_base")
