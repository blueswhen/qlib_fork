...
from rolling_dnn_alpha158_latest import run as run_dnn
...
VALIDATION_RESULTS_PATH = BASE_DIR / "alpha360_alpha158_dnn_strict_validation_results.csv"
VALIDATION_BEST_PATH = BASE_DIR / "alpha360_alpha158_dnn_strict_validation_best.txt"
FINAL_TEST_PATH = BASE_DIR / "alpha360_alpha158_dnn_strict_final_test.txt"
...
SEQ_WEIGHTS = [0.90, 0.95]

H5_LABEL = {"label": ["Ref($close, -6) / Ref($close, -1) - 1"]}

DNN_TRIALS = [
    {
        "trial_name": "dnn01_techA_base_s240",
    ...
    {
        "trial_name": "dnn02_techA_wide_s240",
        "handler_class": "TechAlpha158A",
        "handler_module_path": "qlib.contrib.data.custom_handler",
        "handler_kwargs_extra": None,
        "model_key": "dnn_wide",
...
def _evaluate_signal(
    ...
    *,
    ...
    if fast_mode:
        ...
        indicators_normal = indicator_dict["1day"][0]
        excess_return_with_cost = risk_analysis(
            report_normal["return"] - report_normal["bench"] - report_normal["cost"],
            freq="1day",
        )
        indicator_analysis(indicators_normal)

        result = {
            "recorder_id": f"local:{recorder_name}",
            "IC": ic.mean(),
            "Rank IC": ric.mean(),
            "with_cost_ann_return": excess_return_with_cost.loc["annualized_return", "risk"],
            "with_cost_ir": excess_return_with_cost.loc["information_ratio", "risk"],
            "with_cost_mdd": excess_return_with_cost.loc["max_drawdown", "risk"],
        }
...
def _result_path_for_dnn(trial: dict, stage_suffix: str) -> Path:
    return BASE_DIR / (
        f"rolling_result_dnn_{WINDOW_KEY}_{trial['model_key']}_{trial['handler_class']}_step{int(trial['step'])}_k{TOPK}"
        f"_d1_a{ACCOUNT}_r07_{trial['trial_name']}_{stage_suffix}.txt"
    )


def _run_dnn_trial(trial: dict, eval_segment: str, stage_suffix: str) -> dict:
    result_path = _result_path_for_dnn(trial, stage_suffix)
    run_dnn(
        step=int(trial["step"]),
        topk=TOPK,
        n_drop=1,
...
def main():
    ...
    rows: list[dict] = [
        {
            "trial_name": "strict_dnn_fusion_baseline_alstm_only",
    ...
    for trial in DNN_TRIALS:
        dnn_validation_result = _run_dnn_trial(trial, eval_segment="valid", stage_suffix="strict_valid")
        tab_valid_pred, tab_valid_label = _load_signal_from_recorder(dnn_validation_result["recorder_id"])
        ...
        for mode in FUSION_MODES:
            ...
            for seq_weight in SEQ_WEIGHTS:
                ...
                row = {
                    "trial_name": f"strict_dnn_fusion_{trial['trial_name']}_{mode}_sw{str(seq_weight).replace('.', '')}",
                    "base_model_trial": alstm_validation["model_trial"],
                    "base_strategy_trial": alstm_validation["strategy_trial"],
                ...
                try:
                    ...
                    metrics = _evaluate_signal(
                        ...
                        experiment_name="alpha360_alpha158_dnn_strict_validation_eval",
                    )
                    row.update(
                        {
                            "status": "success",
                            ...
                            "tabular_validation_result_path": str(_result_path_for_dnn(trial, "strict_valid")),
                            "tabular_validation_recorder_id": dnn_validation_result["recorder_id"],
                        }
                    )
    ...
    if not success_rows:
        raise RuntimeError("no successful dnn-fusion validation rows")
    ...
    selected_trial = next(trial for trial in DNN_TRIALS if trial["trial_name"] == best_row["tabular_trial"])
    dnn_test_result = _run_dnn_trial(selected_trial, eval_segment="test", stage_suffix="final_test")
    seq_test_pred, seq_test_label = _load_signal_from_cache(alstm_final["test_cache_path"])
    tab_test_pred, tab_test_label = _load_signal_from_recorder(dnn_test_result["recorder_id"])
    ...
    test_metrics = _evaluate_signal(
        ...
        experiment_name="alpha360_alpha158_dnn_strict_final_test_eval",
    )
    test_metrics.update(
        {
            "cache_path": str(test_cache_path),
            "tabular_test_result_path": str(_result_path_for_dnn(selected_trial, "final_test")),
            "tabular_test_recorder_id": dnn_test_result["recorder_id"],
        }
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Strict Alpha360(ALSTM) + Alpha158(DNN) fusion.")
    parser.parse_args()
    main()
