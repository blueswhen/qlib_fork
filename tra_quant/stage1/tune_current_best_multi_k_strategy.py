...
VALIDATION_RESULTS_PATH = BASE_DIR / "alpha360_tra_fundamental_xgb_multi_k_strategy_validation_results.csv"
VALIDATION_BEST_PATH = BASE_DIR / "alpha360_tra_fundamental_xgb_multi_k_strategy_validation_best.txt"
FINAL_TEST_PATH = BASE_DIR / "alpha360_tra_fundamental_xgb_multi_k_strategy_final_test.txt"
    ...
    return [
        {
            ...
            "kwargs_extra": dict(base_kwargs),
        },
        {
            "trial_name": "multik_eq_s06_a05",
            ...
            "kwargs_extra": {
                **base_kwargs,
                "topk_list": [3, 5, 10, 20],
                "sleeve_weights": [1.0, 1.0, 1.0, 1.0],
                "consensus_strength": 0.6,
                "allocation_blend": 0.5,
            },
        },
        {
            "trial_name": "multik_front_s06_a05",
            "strategy_class": "MultiTopkConsensusPracticalTopkDropoutStrategy",
            "strategy_module_path": "qlib.contrib.strategy.custom_signal_strategy",
            "n_drop": common_kwargs["n_drop"],
            "risk_degree": common_kwargs["risk_degree"],
            "kwargs_extra": {
                **base_kwargs,
                "topk_list": [3, 5, 10, 20],
                "sleeve_weights": [0.4, 0.3, 0.2, 0.1],
                "consensus_strength": 0.6,
                "allocation_blend": 0.5,
            },
        },
        {
            "trial_name": "multik_mid_s06_a05",
            "strategy_class": "MultiTopkConsensusPracticalTopkDropoutStrategy",
            "strategy_module_path": "qlib.contrib.strategy.custom_signal_strategy",
            "n_drop": common_kwargs["n_drop"],
            "risk_degree": common_kwargs["risk_degree"],
            "kwargs_extra": {
                **base_kwargs,
                "topk_list": [3, 5, 10, 20],
                "sleeve_weights": [0.15, 0.35, 0.35, 0.15],
                "consensus_strength": 0.6,
                "allocation_blend": 0.5,
            },
        },
        {
            "trial_name": "multik_eq_s10_a07",
            "strategy_class": "MultiTopkConsensusPracticalTopkDropoutStrategy",
            "strategy_module_path": "qlib.contrib.strategy.custom_signal_strategy",
            "n_drop": common_kwargs["n_drop"],
            "risk_degree": common_kwargs["risk_degree"],
...
def _evaluate_trial(manifest: dict, trial: dict, pred: pd.DataFrame, label: pd.DataFrame, eval_segment: str) -> dict:
    ...
    recorder_name = f"current_best_multi_k_{eval_segment}_{trial['trial_name']}"
    with R.start(experiment_name=f"current_best_multi_k_{eval_segment}_eval", recorder_name=recorder_name):
        rec = R.get_recorder()
...
def main():
    ...
    for idx, trial in enumerate(trials, start=1):
        ...
        try:
            ...
            row = {**trial, **result, "status": "success"}
            rows = [existing for existing in rows if existing.get("trial_name") != trial["trial_name"]]
            rows.append(row)
            _persist(rows, VALIDATION_RESULTS_PATH)
            print(
                "[done] "
                f"{trial['trial_name']} ann={result['with_cost_ann_return']:.6f} "
                f"ir={result['with_cost_ir']:.6f} mdd={result['with_cost_mdd']:.6f} "
                f"score={result['score']:.6f}",
                flush=True,
            )
        except Exception as exc:
            row = {**trial, "status": "failed", "error": repr(exc)}
            rows = [existing for existing in rows if existing.get("trial_name") != trial["trial_name"]]
            rows.append(row)
            _persist(rows, VALIDATION_RESULTS_PATH)
            print(f"[fail] {trial['trial_name']} -> {exc!r}", flush=True)
            raise

    success_rows = [row for row in rows if row.get("status") == "success"]
    if not success_rows:
        raise RuntimeError("no successful validation rows for multi-k strategy experiment")

    best_row = max(success_rows, key=lambda row: float(row["score"]))
    _write_validation_best(best_row, manifest)
    print(
        "[best-valid] "
        f"{best_row['trial_name']} ann={float(best_row['with_cost_ann_return']):.6f} "
        f"ir={float(best_row['with_cost_ir']):.6f} mdd={float(best_row['with_cost_mdd']):.6f} "
        f"score={float(best_row['score']):.6f}",
        flush=True,
    )

    test_result = _evaluate_trial(manifest, best_row, test_pred, test_label, eval_segment="test")
    _write_final_test(best_row, test_result, manifest)
    print(
        "[final-test] "
        f"{best_row['trial_name']} ann={float(test_result['with_cost_ann_return']):.6f} "
