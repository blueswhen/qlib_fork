...
CURRENT_BEST_FINAL_PATH = BASE_DIR / "alpha360_tra_fundamental_dnn_weight_refine_final_test.txt"

VALIDATION_RESULTS_PATH = BASE_DIR / "alpha360_tra_fundamental_v2_xgb_weight_refine_validation_results.csv"
VALIDATION_BEST_PATH = BASE_DIR / "alpha360_tra_fundamental_v2_xgb_weight_refine_validation_best.txt"
FINAL_TEST_PATH = BASE_DIR / "alpha360_tra_fundamental_v2_xgb_weight_refine_final_test.txt"
...
FUNDAMENTAL_FEATURE_PATH = (
    ...
    "csi300_daily_fundamental_features_v2_rankpct_qlib.parquet"
)

SELECTED_TRIAL = {
    "trial_name": "xgb03_techA3d_fund_v2_rankpct_base_s240",
...
FUSION_MODE = "zscore"
TAB_WEIGHTS = [0.05, 0.10, 0.15, 0.20, 0.25, 0.30]
XGB_VALID_STAGE_SUFFIX = "tra_fund_v2_rankpct_fusion_valid"
XGB_TEST_STAGE_SUFFIX = "tra_fund_v2_rankpct_fusion_test"


def _result_path_for_xgb(trial: dict, stage_suffix: str) -> Path:
    return BASE_DIR / (
...
def _run_xgb_trial(trial: dict, eval_segment: str, stage_suffix: str) -> dict:
    result_path = _result_path_for_xgb(trial, stage_suffix)
    run_xgb(
        step=int(trial["step"]),
        topk=TOPK,
        n_drop=1,
...
def main():
    ...
    xgb_validation_result = _run_xgb_trial(
        SELECTED_TRIAL, eval_segment="valid", stage_suffix=XGB_VALID_STAGE_SUFFIX
    )
    ...
    for tab_weight in TAB_WEIGHTS:
        ...
        try:
            ...
            metrics = _evaluate_signal(
                ...
                experiment_name="alpha360_tra_fundamental_v2_xgb_weight_refine_validation_eval",
            )
            row.update(
                {
                    "status": "success",
                    ...
                    "tabular_validation_result_path": str(_result_path_for_xgb(SELECTED_TRIAL, XGB_VALID_STAGE_SUFFIX)),
    ...
    xgb_test_result = _run_xgb_trial(SELECTED_TRIAL, eval_segment="test", stage_suffix=XGB_TEST_STAGE_SUFFIX)
    ...
    test_metrics = _evaluate_signal(
        ...
        experiment_name="alpha360_tra_fundamental_v2_xgb_weight_refine_final_test_eval",
    )
    test_metrics.update(
        {
            "cache_path": str(test_cache_path),
            "tabular_test_result_path": str(_result_path_for_xgb(SELECTED_TRIAL, XGB_TEST_STAGE_SUFFIX)),
