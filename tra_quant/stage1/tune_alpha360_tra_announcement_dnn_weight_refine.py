...
VALIDATION_RESULTS_PATH = BASE_DIR / "alpha360_tra_announcement_dnn_weight_refine_validation_results.csv"
VALIDATION_BEST_PATH = BASE_DIR / "alpha360_tra_announcement_dnn_weight_refine_validation_best.txt"
FINAL_TEST_PATH = BASE_DIR / "alpha360_tra_announcement_dnn_weight_refine_final_test.txt"
...
SELECTED_TRIAL = {
    "trial_name": "dnn04_techA3d_ann_wide_s240",
    "handler_class": "TechAlpha158A3DAnnouncement",
    "handler_module_path": "qlib.contrib.data.custom_handler",
    "handler_kwargs_extra": {
        "announcement_feature_path": ANNOUNCEMENT_FEATURE_PATH,
    },
    "model_key": "dnn_wide",
    "step": 240,
}
FUSION_MODE = "zscore"
TAB_WEIGHTS = [0.02, 0.03, 0.05, 0.08, 0.10, 0.12, 0.15, 0.20, 0.25]
DNN_VALID_STAGE_SUFFIX = "tra_ann_fusion_valid"
DNN_TEST_STAGE_SUFFIX = "tra_ann_fusion_test"
...
def main():
    ...
    dnn_validation_result = _run_dnn_trial(
        SELECTED_TRIAL, eval_segment="valid", stage_suffix=DNN_VALID_STAGE_SUFFIX
    )
    ...
    for tab_weight in TAB_WEIGHTS:
        ...
        row = {
            "trial_name": f"weight_refine_tra_{SELECTED_TRIAL['trial_name']}_{FUSION_MODE}_tw{str(tab_weight).replace('.', '')}",
            "base_model_trial": tra_validation["model_trial"],
            "base_strategy_trial": tra_validation["strategy_trial"],
        ...
        try:
            ...
            metrics = _evaluate_signal(
                ...
                experiment_name="alpha360_tra_announcement_dnn_weight_refine_validation_eval",
            )
            row.update(
                {
                    "status": "success",
                    ...
                    "tabular_validation_result_path": str(_result_path_for_dnn(SELECTED_TRIAL, DNN_VALID_STAGE_SUFFIX)),
    ...
    dnn_test_result = _run_dnn_trial(SELECTED_TRIAL, eval_segment="test", stage_suffix=DNN_TEST_STAGE_SUFFIX)
    ...
    test_metrics = _evaluate_signal(
        ...
        experiment_name="alpha360_tra_announcement_dnn_weight_refine_final_test_eval",
    )
    test_metrics.update(
        {
            "cache_path": str(test_cache_path),
            "tabular_test_result_path": str(_result_path_for_dnn(SELECTED_TRIAL, DNN_TEST_STAGE_SUFFIX)),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Local weight refine for Alpha360(TRA) + announcement DNN fusion.")
    parser.parse_args()
    main()
