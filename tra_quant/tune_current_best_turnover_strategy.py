...
VALIDATION_RESULTS_PATH = BASE_DIR / "alpha360_tra_fundamental_xgb_turnover_strategy_validation_results.csv"
VALIDATION_BEST_PATH = BASE_DIR / "alpha360_tra_fundamental_xgb_turnover_strategy_validation_best.txt"
FINAL_TEST_PATH = BASE_DIR / "alpha360_tra_fundamental_xgb_turnover_strategy_final_test.txt"
...
def _evaluate_trial(manifest: dict, trial: dict, pred: pd.DataFrame, label: pd.DataFrame, eval_segment: str) -> dict:
    ...
    recorder_name = f"current_best_turnover_{eval_segment}_{trial['trial_name']}"
    with R.start(experiment_name=f"current_best_turnover_{eval_segment}_eval", recorder_name=recorder_name):
        rec = R.get_recorder()
...
def main():
    ...
    if not success_rows:
        raise RuntimeError("no successful validation rows for turnover strategy experiment")
