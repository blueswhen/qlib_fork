...
from rolling_xgboost_alpha158_latest import run as run_xgb
...
VALIDATION_RESULTS_PATH = BASE_DIR / "alpha360_tra_alpha158_xgb_strict_validation_results.csv"
VALIDATION_BEST_PATH = BASE_DIR / "alpha360_tra_alpha158_xgb_strict_validation_best.txt"
FINAL_TEST_PATH = BASE_DIR / "alpha360_tra_alpha158_xgb_strict_final_test.txt"
...
XGB_TRIALS = [
    {
        "trial_name": "xgb01_techA_base_s240",
    ...
    {
        "trial_name": "xgb02_techA_deep_s240",
        "handler_class": "TechAlpha158A",
        "handler_module_path": "qlib.contrib.data.custom_handler",
        "handler_kwargs_extra": None,
        "model_key": "xgb_deep",
...
def _result_path_for_xgb(trial: dict, stage_suffix: str, result_suffix: str) -> Path:
    return BASE_DIR / (
        f"rolling_result_xgb_{WINDOW_KEY}_{trial['model_key']}_{trial['handler_class']}_step{int(trial['step'])}_k{TOPK}"
        f"_d1_a{ACCOUNT}_r07_{trial['trial_name']}_{stage_suffix}{f'_{result_suffix}' if result_suffix else ''}.txt"
    )


def _run_xgb_trial(trial: dict, eval_segment: str, stage_suffix: str, result_suffix: str) -> dict:
    effective_stage_suffix = f"{stage_suffix}_{result_suffix}" if result_suffix else stage_suffix
    result_path = _result_path_for_xgb(trial, stage_suffix, result_suffix)
    run_xgb(
        step=int(trial["step"]),
        topk=TOPK,
        n_drop=1,
        ...
        exp_suffix=f"{trial['trial_name']}_{effective_stage_suffix}",
...
def _write_validation_best(best_row: dict, tra_validation_path: Path, validation_best_path: Path):
    strict_validation = _parse_summary_file(tra_validation_path)
    lines = [
...
def main(args: argparse.Namespace):
    tra_validation_path = Path(args.tra_validation_best_path).resolve()
    tra_final_path = Path(args.tra_final_test_path).resolve()
    current_best_final_path = Path(args.current_best_final_path).resolve()
    ...
    tra_validation = _parse_summary_file(tra_validation_path)
    tra_final = _parse_summary_file(tra_final_path)
    ...
    rows: list[dict] = [
        {
            "trial_name": "strict_xgb_fusion_baseline_tra_only",
    ...
    for trial in XGB_TRIALS:
        ...
        for tab_weight in TAB_WEIGHTS:
            ...
            runtime_trial_name = (
                f"strict_xgb_fusion_{trial['trial_name']}_{FUSION_MODE}_tw{str(tab_weight).replace('.', '')}"
                f"_{args.result_suffix}"
                if args.result_suffix
                else f"strict_xgb_fusion_{trial['trial_name']}_{FUSION_MODE}_tw{str(tab_weight).replace('.', '')}"
            )
            row = {
                "trial_name": runtime_trial_name,
                "base_model_trial": tra_validation["model_trial"],
                "base_strategy_trial": tra_validation["strategy_trial"],
            ...
            try:
                ...
                metrics = _evaluate_signal(
                    ...
                    experiment_name="alpha360_tra_alpha158_xgb_strict_validation_eval",
                )
                row.update(
                    {
                        "status": "success",
                        ...
                        "tabular_validation_result_path": str(
                            _result_path_for_xgb(trial, "strict_valid", args.result_suffix)
                        ),
    ...
    if not success_rows:
        raise RuntimeError("no successful xgb-fusion validation rows")

    best_row = max(success_rows, key=lambda item: item["score"])
    _write_validation_best(best_row, tra_validation_path, validation_best_path)
    ...
    if best_row["tabular_trial"] == "none":
        ...
        _write_final_test(
            ...
            tra_final,
            current_best_final,
            tra_validation_path=tra_validation_path,
            final_test_path=final_test_path,
        )
        return

    selected_trial = next(trial for trial in XGB_TRIALS if trial["trial_name"] == best_row["tabular_trial"])
    xgb_test_result = _run_xgb_trial(
        selected_trial,
        eval_segment="test",
        stage_suffix="final_test",
        result_suffix=args.result_suffix,
    )
    ...
    test_metrics = _evaluate_signal(
        ...
        experiment_name="alpha360_tra_alpha158_xgb_strict_final_test_eval",
    )
    test_metrics.update(
        {
            "cache_path": str(test_cache_path),
            "tabular_test_result_path": str(_result_path_for_xgb(selected_trial, "final_test", args.result_suffix)),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Strict Alpha360(TRA) + Alpha158(XGBoost) fusion.")
    ...
    parser.add_argument("--current-best-final-path", default=str(CURRENT_BEST_FINAL_PATH))
