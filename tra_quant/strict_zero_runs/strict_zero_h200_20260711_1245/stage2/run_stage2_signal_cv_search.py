...
def _build_cv_signal_candidates(seq_pred: pd.DataFrame, seq_label: pd.DataFrame, *, profile: str) -> list[dict]:
    ...
    elif profile == "valuation-deep":
        names = ("low_valuation", "value_quality", "cash_quality", "quality_balance")
        linear_weights = (0.005, 0.01, 0.02, 0.03, 0.05)
        low_specs = tuple(
            (low_q, penalty)
            for low_q in (0.15, 0.20, 0.25, 0.30)
            for penalty in (0.05, 0.10, 0.15, 0.20)
        )
        high_specs = ((0.85, 0.02), (0.90, 0.05))
        two_sided_specs = tuple(
            (low_q, penalty, high_q, bonus)
            for low_q, penalty in ((0.20, 0.10), (0.25, 0.10), (0.30, 0.15))
            for high_q, bonus in ((0.85, 0.02),)
        )
    elif profile == "valuation-quick":
        names = ("low_valuation",)
        linear_weights = ()
        low_specs = tuple(
            (low_q, penalty)
            for low_q in (0.18, 0.20, 0.22)
            for penalty in (0.08, 0.10, 0.12, 0.15)
        )
        high_specs = ()
        two_sided_specs = ()
    elif profile == "cash-quality-quick":
        names = ("cash_quality",)
        linear_weights = (0.005, 0.01, 0.015, 0.02, 0.03)
        low_specs = ()
        high_specs = ()
        two_sided_specs = ()
...
def _strategy_trials(profile: str) -> list[dict]:
    ...
    if profile == "stable-wide":
        trials = []
        for margin_tag, score_margin in (("m000", 0.0), ("m025", 0.0025), ("m050", 0.005)):
            for hold_thresh in (3, 4, 5, 7, 10):
                for risk_degree in (0.70, 0.80, 0.85, 0.90):
                    for n_drop in (1, 2):
                        trials.append(
                            stage2._prac(
                                (
                                    f"prac_{margin_tag}{stage2._n_drop_tag(n_drop)}"
                                    f"_hold{hold_thresh}_{stage2._risk_tag(risk_degree)}"
                                ),
                                risk_degree,
                                score_margin,
                                hold_thresh,
        ...
        return trials
    if profile == "stable-focused":
        trials = []
        for margin_tag, score_margin in (("m000", 0.0), ("m025", 0.0025)):
            for hold_thresh in (4, 5, 7, 10):
                for risk_degree in (0.70, 0.80, 0.85, 0.90):
                    trials.append(
                        stage2._prac(
        ...
        return trials
    if profile == "stable-quick":
        trials = []
        for hold_thresh in (4, 5, 6, 7):
            for risk_degree in (0.65, 0.70, 0.75, 0.80, 0.85, 0.90, 0.95):
                trials.append(
                    stage2._prac(
                        f"prac_m000_hold{hold_thresh}_{stage2._risk_tag(risk_degree)}",
                        risk_degree,
        ...
        return trials
    if profile == "stable-risk-fine":
        trials = []
        margins = (("m000", 0.0), ("m010", 0.0010), ("m015", 0.0015), ("m020", 0.0020), ("m025", 0.0025))
        risk_degrees = (0.700, 0.705, 0.710, 0.715, 0.720, 0.725, 0.730, 0.735, 0.740, 0.745, 0.750)
        for margin_tag, score_margin in margins:
            for hold_thresh in (5, 6):
        ...
        return trials
    if profile == "stable-quarter":
        trials = []
        margins = (("m000", 0.0), ("m010", 0.0010), ("m025", 0.0025))
        for margin_tag, score_margin in margins:
            for hold_thresh in (4, 5, 6, 7, 8, 10, 12):
                for risk_degree in (0.70, 0.75, 0.80, 0.85, 0.90, 0.95):
                    for n_drop in (1, 2):
                        trials.append(
                            stage2._prac(
                                (
        ...
        return trials
    if profile == "stable-cash-quick":
        trials = []
        for hold_thresh in (5, 7, 10):
