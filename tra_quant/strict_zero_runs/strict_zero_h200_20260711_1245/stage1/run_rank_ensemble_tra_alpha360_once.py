...
def load_existing_validation_rows(output_csv_path: Path, extra_paths: list[Path] | tuple[Path, ...] = ()):
    row_map = {}
    for path in (output_csv_path, *extra_paths, VALID_GRID_PATH, LEGACY_VALID_GRID_PATH):
        if not path.exists():
            continue
        df = pd.read_csv(path)
...
def evaluate_or_reuse_validation_rows(
    ...
    *,
    ...
    reuse_grid_paths: list[Path] | tuple[Path, ...] = (),
):
    existing_rows = {} if force_recompute else load_existing_validation_rows(output_csv_path, reuse_grid_paths)
    valid_rows_out = []
    reused_count = 0
    evaluated_count = 0
...
def main():
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Existing validation grid CSV to reuse in addition to this shard's output grid. Repeatable.",
    )
    ...
    valid_df, reused_count, evaluated_count = evaluate_or_reuse_validation_rows(
        ...
        reuse_grid_paths=[Path(path).expanduser().resolve() for path in args.reuse_grid_path],
    )
