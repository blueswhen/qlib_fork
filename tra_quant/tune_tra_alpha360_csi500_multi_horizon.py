...
VALIDATION_RESULT_PATH = BASE_DIR / "tra_csi500_multi_horizon_validation_results.csv"
VALIDATION_BEST_PATH = BASE_DIR / "tra_csi500_multi_horizon_validation_best.txt"
FINAL_TEST_PATH = BASE_DIR / "tra_csi500_multi_horizon_final_test.txt"
...
TRIALS = [
    {
        "trial_name": "csi500_tra_lstm_s240_h3_ref",
        "family": "single_horizon",
    ...
    {
        "trial_name": "csi500_tra_lstm_s240_h5_ref",
        "family": "single_horizon",
        "model_key": "tra_lstm_base",
        "step": 240,
        "handler_kwargs_extra": {"label": H5_LABEL},
        "model_kwargs_override": None,
    },
    {
        "trial_name": "csi500_mhtra_lstm_s240_h35_eq",
    ...
    {
        "trial_name": "csi500_mhtra_lstm_s240_h3510_front",
        "family": "multi_horizon",
        "model_key": "mh_tra_lstm_base",
        "step": 240,
...
def _current_baseline_test_metrics() -> dict:
    if not CURRENT_BASELINE_MANIFEST.exists():
        return {}
    data = json.loads(CURRENT_BASELINE_MANIFEST.read_text(encoding="utf-8"))
    current_best = data.get("current_best_baseline", {})
    return current_best.get("final_test", data.get("test_metrics", {}))
...
def _write_validation_best(best_row: dict):
    ...
    lines = [
        f"selection_scope=csi500_multi_horizon_tra_validation_only",
...
def _write_final_test(best_row: dict, test_result: dict):
    ...
    baseline_ir = baseline.get("with_cost_ir")
    baseline_mdd = baseline.get("with_cost_mdd")
    ...
    lines = [
        ...
        f"reference_csi300_current_best_ann={baseline_ann}",
        f"reference_csi300_current_best_ir={baseline_ir}",
        f"reference_csi300_current_best_mdd={baseline_mdd}",
    ]
    if baseline_ann is not None:
        lines.append(f"delta_vs_csi300_ann={float(test_result['with_cost_ann_return']) - float(baseline_ann)}")
    if baseline_ir is not None:
        lines.append(f"delta_vs_csi300_ir={float(test_result['with_cost_ir']) - float(baseline_ir)}")
    if baseline_mdd is not None:
        lines.append(f"delta_vs_csi300_mdd={float(test_result['with_cost_mdd']) - float(baseline_mdd)}")
...
def main(trial_pattern: str | None = None, max_trials: int | None = None, skip_final_test: bool = False):
    ...
    if not success_rows:
        raise RuntimeError("no successful validation rows for csi500 multi-horizon TRA experiment")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run strict csi500 Alpha360 + TRA multi-horizon experiments.")
